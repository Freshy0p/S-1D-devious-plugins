package net.unethicalite.fletcher.utils;
...
public class S1dBank {
    ...
    public static void depositAll(boolean delay, Predicate<Item> filter) {
        Set<Item> items =
                Bank.Inventory.getAll(filter).stream()
                        .filter(S1dPredicates.distinctByProperty(Item::getId))
    ...
    public static boolean withdrawEquipment(ConfigList equipment) {
        ...
        List<Item> equipmentItems =
                Bank.Inventory.getAll(S1dPredicates.itemConfigList(equipment, true, false));
